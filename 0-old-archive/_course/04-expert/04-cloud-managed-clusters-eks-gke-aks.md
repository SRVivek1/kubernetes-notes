---
layout: lesson
title: "Cloud-Managed Clusters: EKS, GKE, and AKS"
seo_title: "Kubernetes EKS GKE AKS: IRSA, Workload Identity Failures"
...
permalink: /course/expert/cloud-managed-clusters-eks-gke-aks/
level: expert
module: 4
lesson: 4
order: 404
prerequisites:
  - low-level-networking-and-packet-capture
keywords: [eks troubleshooting, gke troubleshooting, aks troubleshooting, irsa, workload identity, azure ad workload identity, cluster autoscaler, cloudwatch container insights]
tags: [eks, gke, aks, cloud, irsa, workload-identity, autoscaler]
...
> **Prerequisites:** This builds on [Low-Level Networking and Packet Capture](/course/expert/low-level-networking-and-packet-capture/). You should already understand ServiceAccounts and RBAC from Intermediate, and admission webhook mechanics from the [Admission Control](/course/expert/admission-control-and-webhook-failures/) lesson in this level, since IRSA and Workload Identity are themselves implemented via mutating webhooks.
...
| Finding | Go to |
|---|---|
| IAM/Workload Identity trust relationship is wrong | Review the [Admission Control](/course/expert/admission-control-and-webhook-failures/) webhook health checks — the injection webhook must be healthy first |
| Cloud storage (EBS/PD/Disk) CSI driver issue | Advanced/Intermediate storage material |
| GitOps deploy changed the ServiceAccount annotation | [GitOps, Progressive Delivery & Rollback](/course/expert/gitops-progressive-delivery-and-rollback/) |
