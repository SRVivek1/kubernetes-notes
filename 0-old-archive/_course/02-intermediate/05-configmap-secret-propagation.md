---
layout: lesson
title: "ConfigMap and Secret Propagation"
seo_title: "Kubernetes ConfigMap Propagation Delay & Spring Cloud Config"
...
permalink: /course/intermediate/configmap-secret-propagation/
level: intermediate
module: 2
lesson: 5
order: 205
...
keywords: [configmap propagation delay, kubernetes env var immutable, spring cloud config server, vault kubernetes, configmap volume mount, kubelet sync period, kubectl diff]
tags: [configmap, secrets, spring-cloud-config, vault, configuration]
...
> **Prerequisites:** This lesson assumes you've completed [DNS and Service Discovery Deep Dive](/course/intermediate/dns-and-service-discovery-deep-dive/) and already know the Beginner-level basics of creating a ConfigMap and referencing it via `envFrom`/`env` or `volumeMounts` in a pod spec.
...
- **Config Server itself unreachable** — a Kubernetes Service/DNS/NetworkPolicy issue underneath (see the [DNS deep dive](/course/intermediate/dns-and-service-discovery-deep-dive/)) manifests here as "app fails to start, no obvious Kubernetes-level cause," because the actual dependency is another in-cluster service, not a ConfigMap object.
...
- **Vault token/auth failure** — Vault-backed config requires the pod to authenticate (often via its ServiceAccount token, using Vault's Kubernetes auth method); a token permission problem here looks like a config problem but is actually an RBAC/auth problem — see the [Namespaces, RBAC & Multi-Tenancy lesson](/course/intermediate/namespaces-rbac-and-multi-tenancy/) for the ServiceAccount side of this.
