---
layout: lesson
title: "Service Mesh Troubleshooting with Istio"
seo_title: "Istio Service Mesh Troubleshooting: mTLS, Circuit Breakers, 503s"
...
permalink: /course/advanced/service-mesh-troubleshooting-istio/
level: advanced
module: 3
lesson: 7
order: 307
prerequisites:
  - low-level-networking-cni-and-kube-proxy
keywords: [istio troubleshooting, envoy access log flags, uf uc uo, istio circuit breaker, destinationrule virtualservice, mtls debugging, nginx ingress controller, outlier detection]
tags: [istio, service-mesh, envoy, ingress, circuit-breaker, spring-boot]
...
> **Prerequisites:** Complete [Low-Level Networking: CNI and kube-proxy Internals](/course/advanced/low-level-networking-cni-and-kube-proxy/) first. A full Istio install (`istioctl install`) is needed to run every command in this lesson hands-on — if you don't have one available, read through for the mental model and command shapes, which the lesson's Lab section also calls out explicitly.
