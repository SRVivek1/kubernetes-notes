---
layout: lesson
title: "Observability: Metrics, Logs, Traces, and Autoscaling"
seo_title: "Kubernetes Observability: Prometheus, Tracing & HPA for Java Apps"
...
permalink: /course/advanced/observability-metrics-logs-traces/
level: advanced
module: 3
lesson: 8
order: 308
prerequisites:
  - service-mesh-troubleshooting-istio
keywords: [promql spring boot metrics, distributed tracing jaeger, hpa custom metrics, vertical pod autoscaler, poddisruptionbudget, micrometer prometheus, cluster autoscaler]
tags: [observability, prometheus, grafana, tracing, hpa, vpa, autoscaling]
...
> **Prerequisites:** Complete [Service Mesh Troubleshooting with Istio](/course/advanced/service-mesh-troubleshooting-istio/) first. Familiarity with basic Prometheus/Grafana concepts from Intermediate is assumed but not required in depth.
...
Common Java-workload-specific resourcing mistake: setting CPU **limits** too low causes CFS throttling, which shows up as latency spikes, not OOM — this is exactly the [GC-tuning-vs-throttling distinction](/course/advanced/gc-tuning-and-cpu-throttling/) from earlier in this module, and it's worth checking `cpu.stat` again here whenever HPA scaling on CPU utilization looks erratic, since throttled pods report misleadingly low "usable" CPU.
