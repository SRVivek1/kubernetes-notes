...
heading: "Capstone: Full Game Day"
title: "Kubernetes Capstone: Multi-Layered Incident Game Day"
subtitles:
  - "Run the Expert-level capstone: an unannounced multi-layered Kubernetes failure combining NetworkPolicy, IRSA, and node pressure, evaluated end to end."
permalink: /kubernetes/capstone-full-game-day
seo:
  type: TechArticle
...
Passing this capstone is the practical definition of "incident commander ready" used throughout this course's [Assessment Rubric](/kubernetes/assessment-rubric): not "knows every command," but "can independently drive an incident from alert to resolution to written postmortem, under time pressure, without being told what's broken."

{% capture callout_body %}
This capstone assumes you've completed every prior lesson in this level, especially [Incident Command and Postmortems](/kubernetes/incident-command-and-postmortems), [Chaos Engineering and Failure Injection](/kubernetes/chaos-engineering-and-failure-injection), [Cloud-Managed Clusters](/kubernetes/cloud-managed-clusters-eks-gke-aks), and [Low-Level Networking and Packet Capture](/kubernetes/low-level-networking-and-packet-capture). If any of those feel shaky, go back, this exercise is deliberately unforgiving of gaps.
{% endcapture %}
...
This isn't a checklist of "did they type the right kubectl command", it's an evaluation of incident-command judgment under ambiguity. Score against the same dimensions as the [Assessment Rubric](/kubernetes/assessment-rubric), with capstone-specific emphasis on:
...
7. **Score against the evaluation table above** and the course's [Assessment Rubric](/kubernetes/assessment-rubric), and identify specific follow-up lessons (from anywhere in this course, not just Expert) to revisit based on where the trainee struggled.
