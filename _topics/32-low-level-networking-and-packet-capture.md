...
heading: "Low-Level Networking and Packet Capture"
title: "Kubernetes Networking Internals: iptables, CNI, Packet Capture"
subtitles:
  - "Go beyond single-node CNI debugging into cluster-wide network architecture: iptables/IPVS internals, Calico/Cilium, and packet capture for cross-node failures."
permalink: /kubernetes/low-level-networking-and-packet-capture
seo:
  type: TechArticle
...
This builds on [Admission Control and Webhook Failures](/kubernetes/admission-control-and-webhook-failures). You should already be comfortable with the single-node CNI debugging workflow from the Advanced networking lesson (`/kubernetes/`), checking CNI pod health, basic `NetworkPolicy` existence, and Service/Endpoint mismatches, before tackling cross-node packet-level analysis here.
{% endcapture %}
...
| Finding | Go to |
|---|---|
| Node itself is under resource pressure, not a network issue | [Node and Control Plane Internals](/kubernetes/node-and-control-plane-internals) |
| Cloud provider security groups/firewall rules are the actual block | [Cloud-Managed Clusters](/kubernetes/cloud-managed-clusters-eks-gke-aks) |
