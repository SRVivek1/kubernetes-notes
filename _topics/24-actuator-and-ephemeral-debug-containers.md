...
heading: "Actuator Runbooks and Ephemeral Debug Containers"
title: "Spring Boot Actuator Runbooks & kubectl debug for Kubernetes"
subtitles:
  - "Build Actuator-driven diagnostic runbooks, change log levels without redeploying, and debug distroless Spring Boot pods using kubectl ephemeral containers."
permalink: /kubernetes/actuator-and-ephemeral-debug-containers
seo:
  type: TechArticle
...
Complete [GC Tuning and CPU Throttling in Containers](/kubernetes/gc-tuning-and-cpu-throttling) first. You should already be fluent with `jcmd`, `kubectl exec`, and `kubectl cp` from the earlier lessons in this module.
{% endcapture %}
