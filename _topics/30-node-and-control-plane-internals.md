...
heading: "Node and Control Plane Internals"
title: "Kubernetes Node Internals: kubelet, containerd, crictl, etcd"
subtitles:
  - "Learn how kubelet, containerd, and etcd interact under the hood so you can diagnose node pressure and control-plane health as a Kubernetes on-call lead."
permalink: /kubernetes/node-and-control-plane-internals
seo:
  type: TechArticle
...
This lesson assumes you've completed the [Advanced capstone](/kubernetes/capstone-production-incident-simulation) and are comfortable reading `kubectl describe pod`, pod status, and cluster events without prompting. If node conditions, taints, or `kubectl drain` are unfamiliar territory, skim the Intermediate scheduling material first, this lesson goes straight to the internals, not the basics.
{% endcapture %}
...
| Finding | Go to |
|---|---|
| Node pressure is causing pod evictions across namespaces | [Persistent Storage for Stateful Workloads](/kubernetes/persistent-storage-for-stateful-workloads) (ephemeral storage/disk pressure), [Observability: Metrics, Logs, Traces, and Autoscaling](/kubernetes/observability-metrics-logs-traces) (HPA/VPA/PDB) |
| Node is fine but you suspect the CNI/network plugin | [Low-Level Networking and Packet Capture](/kubernetes/low-level-networking-and-packet-capture) |
| Cloud-managed node group (EKS/GKE/AKS) specific issue | [Cloud-Managed Clusters](/kubernetes/cloud-managed-clusters-eks-gke-aks) |
