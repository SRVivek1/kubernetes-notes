...
heading: "Thread Dumps and Deadlock Analysis"
title: "Kubernetes Thread Dump Analysis for Java & Spring Boot Apps"
subtitles:
  - "Learn to capture and read Java thread dumps in Kubernetes pods, diagnose deadlocks and connection-pool exhaustion, and correlate high CPU with specific threads."
permalink: /kubernetes/thread-dumps-and-deadlock-analysis
seo:
  type: TechArticle
...
You should have completed the [Intermediate capstone](/kubernetes/capstone-on-call-simulation) and be comfortable with `kubectl exec`, `kubectl cp`, and basic `jcmd` usage from that level.
{% endcapture %}
...
- Threads stuck in DNS resolution (`java.net.InetAddress`) → a CoreDNS or network issue, not a JVM problem, see the [low-level networking lesson](/kubernetes/low-level-networking-cni-and-kube-proxy).
