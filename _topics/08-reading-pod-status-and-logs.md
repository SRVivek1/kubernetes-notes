...
heading: "Reading Pod Status and Logs"
title: "Kubernetes Pod Status, Logs & Restart Diagnosis Guide"
subtitles:
  - "Master the kubectl describe, logs, and events triage trio to diagnose Kubernetes pod status, restart counts, and OOMKilled failures for Spring Boot apps."
permalink: /kubernetes/reading-pod-status-and-logs
seo:
  type: TechArticle
...
[Kubernetes Architecture Fundamentals](/kubernetes/kubernetes-architecture-fundamentals), [Pods, ReplicaSets, and Deployments](/kubernetes/pods-replicasets-and-deployments), [Services and Basic Networking](/kubernetes/services-and-basic-networking), [ConfigMaps and Secrets Basics](/kubernetes/configmaps-and-secrets-basics)
{% endcapture %}
...
You're looking for: nodes in `NotReady` or under pressure (`MemoryPressure`, `DiskPressure`), a burst of `Warning` events (`FailedScheduling`, `BackOff`, `Unhealthy`), and whether the affected Pods are isolated to one Deployment/namespace (app-level issue) or spread across the whole cluster (points toward node/network/DNS infrastructure, covered in depth in [Expert](/kubernetes/node-and-control-plane-internals)).
...
`ImagePullBackOff`/`ErrImagePull` and `Pending`-from-scheduling get their own lesson next, [Image Pull and Scheduling Basics](/kubernetes/image-pull-and-scheduling-basics), since they're common enough at this level to deserve dedicated walkthroughs.
...
> Checking restarts across *every* replica of a Deployment at once, and trending restarts over time with Prometheus, is genuinely useful but goes beyond single-pod triage, see [Intermediate: Restart Patterns Across Deployments](/kubernetes/restart-troubleshooting-across-a-deployment) when you're ready for that.
...
The full diagnostic workflow for OOMKilled, checking JVM heap flags against the cgroup limit, native memory tracking, distinguishing a leak from an undersized limit, is covered with resource requests/limits in [Resource Requests and Limits Basics](/kubernetes/resource-requests-and-limits-basics), and the deep JVM-level analysis (heap dumps, off-heap accounting) lives in [Advanced](/kubernetes/heap-dumps-and-memory-leaks).
