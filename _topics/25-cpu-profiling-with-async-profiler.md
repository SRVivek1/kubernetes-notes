...
heading: "CPU Profiling with async-profiler"
title: "CPU Profiling Java Apps in Kubernetes with async-profiler"
subtitles:
  - "Use async-profiler as a sidecar or in-pod agent to capture CPU flame graphs from live Spring Boot pods in Kubernetes without stopping the JVM."
permalink: /kubernetes/cpu-profiling-with-async-profiler
seo:
  type: TechArticle
...
Complete [Actuator Runbooks and Ephemeral Debug Containers](/kubernetes/actuator-and-ephemeral-debug-containers) first, you'll use the same `kubectl cp`/`kubectl exec` fluency, and the same ephemeral-container approach applies if your target image has no shell for unpacking the profiler.
{% endcapture %}
