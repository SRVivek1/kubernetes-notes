...
heading: "Admission Control and Webhook Failures"
title: "Kubernetes Admission Webhook Failures: Blast Radius & Debugging"
subtitles:
  - "Understand how validating and mutating admission webhooks can silently block every deploy cluster-wide, and how to diagnose it fast as an incident lead."
permalink: /kubernetes/admission-control-and-webhook-failures
seo:
  type: TechArticle
...
This lesson builds directly on [Node and Control Plane Internals](/kubernetes/node-and-control-plane-internals). Make sure you're solid on the request-authorization basics from Intermediate RBAC material before continuing, this lesson assumes you already know what `kubectl auth can-i` does.
{% endcapture %}
...
| Finding | Go to |
|---|---|
| ServiceAccount needs cloud IAM permissions (AWS/GCP/Azure) | [Cloud-Managed Clusters](/kubernetes/cloud-managed-clusters-eks-gke-aks) |
| Blocked GitOps deploy caused by a webhook | [GitOps, Progressive Delivery & Rollback](/kubernetes/gitops-progressive-delivery-and-rollback) |
