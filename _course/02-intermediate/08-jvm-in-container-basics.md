---
layout: lesson
title: "JVM-in-Container Basics"
seo_title: "JVM Memory Limits in Kubernetes Containers Explained"
...
permalink: /course/intermediate/jvm-in-container-basics/
level: intermediate
module: 2
lesson: 8
order: 208
...
keywords: [jvm container memory, jvm cgroup limits, xmx kubernetes, jcmd vm.flags, container-aware jvm, jstat, jvm heap sizing kubernetes, oomkilled jvm]
tags: [jvm, memory, cgroups, jcmd, jstat]
...
Every lesson so far has treated the JVM as a black box that either starts or doesn't. This lesson opens that box just enough to explain the interaction that causes a huge share of Java-specific OOM incidents on Kubernetes: the relationship between the container's cgroup memory limit and the JVM's own heap-sizing defaults. You already diagnosed OOMKilled at the symptom level in the [CrashLoopBackOff lesson](/course/intermediate/crashloopbackoff-and-exit-codes/) — this lesson explains the mechanism underneath it, and introduces `jcmd`/`jstat` as basic inspection tools. Full thread dumps, heap dumps, and GC tuning are deliberately out of scope here — that's [Advanced-level material](/course/advanced/thread-dumps-and-deadlock-analysis/); this lesson is strictly the conceptual foundation and baseline identity checks.

> **Prerequisites:** This lesson assumes you've completed [Namespaces, RBAC, and Multi-Tenancy](/course/intermediate/namespaces-rbac-and-multi-tenancy/) and the OOMKilled material in [CrashLoopBackOff and Exit Code Deep Dive](/course/intermediate/crashloopbackoff-and-exit-codes/).
