---
layout: lesson
title: "Capstone: On-Call Simulation"
seo_title: "Kubernetes On-Call Simulation: Multi-Service Incident Lab"
...
permalink: /course/intermediate/capstone-on-call-simulation/
level: intermediate
module: 2
lesson: 9
order: 209
prerequisites:
  - jvm-in-container-basics
keywords: [kubernetes on-call simulation, incident triage kubernetes, spring boot troubleshooting capstone, kubernetes root cause analysis, multi-service outage]
tags: [capstone, incident-response, triage, on-call]
...
> **Prerequisites:** This capstone assumes you've completed all of Module 2, Lessons 1 through 8 — [Probes](/course/intermediate/liveness-readiness-and-startup-probes/), [CrashLoopBackOff & Exit Codes](/course/intermediate/crashloopbackoff-and-exit-codes/), [Restart Troubleshooting Across a Deployment](/course/intermediate/restart-troubleshooting-across-a-deployment/), [DNS & Service Discovery](/course/intermediate/dns-and-service-discovery-deep-dive/), [ConfigMap/Secret Propagation](/course/intermediate/configmap-secret-propagation/), [Persistent Storage](/course/intermediate/persistent-storage-for-stateful-workloads/), [Namespaces & RBAC](/course/intermediate/namespaces-rbac-and-multi-tenancy/), and [JVM-in-Container Basics](/course/intermediate/jvm-in-container-basics/). This is a synthesis exercise, not a place to learn new material — if any of the diagnostic commands feel unfamiliar, go back to the relevant lesson first.
...
1. **Get the lay of the land first** (per the [restart troubleshooting lesson](/course/intermediate/restart-troubleshooting-across-a-deployment/) — scope before you root-cause):
   ```bash
...
3. **For any service that IS crash-looping**, run the full decision tree from [Lesson 2](/course/intermediate/crashloopbackoff-and-exit-codes/):
   ```bash
   kubectl describe pod -n oncall-sim -l app=payment-service
   kubectl logs -n oncall-sim -l app=payment-service --previous
   ```

4. **For the networking suspect**, work the layered checklist from [Lesson 4](/course/intermediate/dns-and-service-discovery-deep-dive/) — DNS resolution first, then direct pod IP, then NetworkPolicy:
   ```bash
...
5. **For the config suspect**, check env vars actually in effect versus the current ConfigMap content, per [Lesson 5](/course/intermediate/configmap-secret-propagation/):
   ```bash
...
6. **For the JVM memory suspect**, confirm the exit code and compare `-Xmx` against the container limit, per [Lesson 8](/course/intermediate/jvm-in-container-basics/):
   ```bash
...
You've completed Intermediate. The [Advanced level](/course/advanced/thread-dumps-and-deadlock-analysis/) picks up exactly where [JVM-in-Container Basics](/course/intermediate/jvm-in-container-basics/) left off — thread dumps, heap dumps, GC tuning, and profiling — plus service mesh and observability at scale.
